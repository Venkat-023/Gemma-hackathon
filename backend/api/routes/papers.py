...
from tasks.paper_processing import process_paper
...
async def upload_paper(
    ...
    return PaperUploadResponse(paper_id=paper.id, title=paper.title, status="processing", task_id=task.id)


@router.post("/arxiv", response_model=PaperUploadResponse)
async def ingest_arxiv(payload: ArxivIngestRequest, db: AsyncSession = Depends(get_db)) -> PaperUploadResponse:
