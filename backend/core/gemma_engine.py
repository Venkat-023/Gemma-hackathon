...
class GemmaEngine:
    def __init__(self, model_name: str = "gemma4:27b", timeout_seconds: int | None = None) -> None:
        self.model_name = model_name
        self.temperature = 0.25
        self.max_retries = 3
        self.settings = get_settings()
        self.client = ollama.Client(host=self.settings.ollama_host, timeout=timeout_seconds or self.settings.gemma_timeout_seconds)
        ...
        def _call() -> str:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "response_tokens_estimate": len(text.split()),
                },
            )
            return text
    ...
    def generate_structured(self, prompt: str, schema_hint: str = "") -> dict[str, Any]:
        ...
        if parsed is not None:
            return parsed
        raise ValueError(f"Gemma returned invalid JSON. Original response: {first_response}")
...
2. Add important scientific entities
3. Extract typed relationships between entities
4. Normalize entity names to canonical form
"""
        return self.generate_structured(prompt)

    def generate_hypothesis(self, context: dict[str, Any]) -> dict[str, Any]:
        prompt = f"""
You are an AI co-scientist specializing in identifying breakthrough research opportunities.
...
CROSS-DOMAIN CONNECTIONS FOUND (papers that don't cite each other but share concepts):
{json.dumps(context.get("cross_domain_connections", []))}

YOUR TASK:
Generate exactly {context.get("num_hypotheses", 5)} scientifically novel research hypotheses that:
...
"""
    ...
    def detect_contradiction(self, text_a: str, text_b: str, topic: str) -> dict[str, Any]:
        prompt = f"""
Analyze these two research excerpts on the topic: "{topic}"
...
Determine if these papers contradict each other. Look specifically for opposite conclusions,
conflicting quantitative results, different causal claims, or methodological incompatibilities.
