...
class ScientificEntityExtractor:
    ...
    def __init__(self, use_gemma_refinement: bool = False) -> None:
        self.nlp = _get_scientific_nlp()
        self.use_gemma_refinement = use_gemma_refinement
        self.gemma = GemmaEngine(get_settings().gemma_light_model) if use_gemma_refinement else None
    ...
    def extract(self, chunks: list[Chunk]) -> EntityExtractionResult:
        ...
        top_entities = {k: list(v)[:10] for k, v in typed.items()}
